...
;; Description: Плагин добавляет возможность быстро собрать список игроков для Телепортирования (находящихся рядом или по смс) и запустить цикл телепортации
...
class TP
{
  ...
  minSMSLVL := 1
  checkSMSInChatlog := 0
  checkNearInChatlog := 0
  checkingNearStringCount := 0
  maxCheckingNearStringCount := 20
  __running := 0
  ...
  startAddingPlayersInListFromSMS(Data)
  {
    ...
    if (!this.checkSMSInChatlog) {
      ...
      addMessageToChatWindow("{FFFF00}Начат сбор игроков по SMS для телепортации")
  ...
  __smsStopAddingPlayersInList()
  {
    if (this.checkSMSInChatlog) {
      ...
      addMessageToChatWindow("{FFFF00}Сбор игроков по SMS для телепортации был остановлен [ в списке " maxCountPlayersInList maxCountSMSPlayersToStop " ]")
    }

...
class Uninvites
{
  ...
  readListFile()
  {
    Global UninvitesListFile

    FileRead, Contents, %A_ScriptDir%\%UninvitesListFile%
    if not ErrorLevel
    {
  ...
  clearListFile()
  {
    ...
    FileAppend, , %A_ScriptDir%\%UninvitesListFile%.tmp
    FileCopy, %A_ScriptDir%\%UninvitesListFile%.tmp, %A_ScriptDir%\%UninvitesListFile%, 1
    FileDelete, %A_ScriptDir%\%UninvitesListFile%.tmp
    ...
    if ((this.runOnline || this.runOffline || this.runOfflineList) && this._playersList.MaxIndex()) {
      if (this.runOnline) {
        if (getPlayerIdByName(this._playersList[1]) >= 0) {
          sendChatMessage("/uninvite " getPlayerIdByName(this._playersList[1]) " !")
        } else {
